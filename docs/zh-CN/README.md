...
<p align="center">
  <a href="https://raids-lab.github.io/orbit/zh/docs/admin/">文档</a> ·
...
Orbit 是一个基于 Kubernetes 的平台，帮助团队管理异构算力资源（例如 GPU），并通过统一调度、开发环境与可观测性能力运行 AI 工作负载。

## 项目来源

Orbit 基于 RAIDS Lab 开源的 [Crater](https://github.com/raids-lab/crater) 项目演进而来。本仓库在 Crater 的基础上继续开发，并将项目介绍、文档、前端页面和部署资产统一调整为 Orbit 平台。
...
如果您希望使用 Kind 快速部署一个基本的 Orbit，请参照[最小化部署](https://raids-lab.github.io/orbit/zh/docs/admin/kind-start/)。

如果您希望在集群中部署一个完整的 Orbit，请参照[集群部署指南](https://raids-lab.github.io/orbit/zh/docs/admin/deploy-on-cluster/)。
...
要开始使用 **Orbit**，您首先需要有一个正在运行的 Kubernetes 集群。您可以使用以下方法之一来设置集群：
...
要在生产环境或大规模测试环境中部署 Orbit，您可以使用任何标准的 Kubernetes 设置。

📖 [https://kubernetes.io/docs/setup/](https://kubernetes.io/docs/setup/)

### 通过 Helm（OCI）安装

> 更完整的步骤请以文档为准。Chart 版本可在 `charts/orbit/Chart.yaml`（字段 `version`）或 GitHub releases 中查看。
...
- `charts/`: 用于部署 Orbit 的 Helm Chart
- `website/`: 文档网站源码
- `grafana-dashboards/`: Orbit 使用的 Grafana Dashboard
- `docs/`: 文档入口与多语言资源
- `hack/`: 开发工具与脚本

## 贡献指南

我们欢迎社区贡献！如果您想为 Orbit 项目做出贡献，请遵循以下流程。

### 1) Fork 与克隆

1. **Fork 仓库**
   - 访问 [Orbit 主仓库](https://github.com/raids-lab/orbit)
   - 点击右上角的 **Fork**
...
Orbit 提供统一的配置管理方式，可将配置集中到单一目录，并为各组件创建软链接。
...
Orbit 使用 Apache License 2.0 许可证，详见 [LICENSE](../../LICENSE)。
