...
运行 `make curd` 会重新生成 `dao/query/*.gen.go`；业务逻辑再落到 handler/service 中。
...
| `ModelExport` | `dao/model/model_export.go` | checkpoint 到可部署模型 artifact 的异步导出任务 |
...
| `ModelDownloadReconciler` | `pkg/reconciler/modeldownload-reconciler.go` | Kubernetes `batch.Job` | 同步模型下载和 checkpoint 模型导出 Job，完成后创建模型数据集 |
