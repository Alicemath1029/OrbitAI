...
**Homepage:** <https://github.com/raids-lab/orbit>
...
* <https://github.com/raids-lab/orbit/tree/main/charts/orbit>
...
| backendConfig | object | `{"auth":{"ldap":{"alias":"","attributeMapping":{"displayName":"cn","email":"mail","username":"uid"},"enable":false,"help":"","server":{"address":"ldap://ldap.example.com:389","baseDN":"dc=example,dc=org","bindDN":"cn=admin,dc=example,dc=org","bindPassword":"<MUSTEDIT>"},"uid":{"ldapAttribute":{"gid":"gidNumber","uid":"uidNumber"},"rid":{"offset":10000,"pgidAttribute":"primaryGroupID","sidAttribute":"objectSid"},"source":"default"}},"normal":{"allowLogin":true,"allowRegister":true},"token":{"accessTokenSecret":"example-access-token","refreshTokenSecret":"example-refresh-token"}},"enableLeaderElection":false,"modelDownload":{"image":"orbit-harbor.act.buaa.edu.cn/docker.io/library/python:3.11-slim"},"port":":8088","postgres":{"TimeZone":"Asia/Shanghai","dbname":"postgres","host":"orbit-postgresql.orbit-system.svc.cluster.local","password":"<MUSTEDIT>","port":5432,"sslmode":"disable","user":"postgres"},"prometheusAPI":"http://192.168.0.1:12345","registry":{"buildTools":{"proxyConfig":{"httpProxy":null,"httpsProxy":null,"noProxy":null}},"enable":false,"harbor":{"password":"<MASKED>","server":"harbor.example.com","user":"admin"}},"secrets":{"imagePullSecretName":"","tlsForwardSecretName":"orbit-tls-forward-secret","tlsSecretName":"orbit-tls-secret"},"smtp":{"enable":false,"host":"mail.example.com","notify":"example@example.com","password":"<MASKED>","port":25,"user":"example"},"storage":{"prefix":{"account":"accounts","public":"public","user":"users"},"pvc":{"readOnlyMany":null,"readWriteMany":"orbit-rw-storage"}}}` | Backend configuration |
...
| backendConfig.modelDownload | object | `{"image":"orbit-harbor.act.buaa.edu.cn/docker.io/library/python:3.11-slim"}` | Model download functionality configurations |
| backendConfig.modelDownload.image | string | `"orbit-harbor.act.buaa.edu.cn/docker.io/library/python:3.11-slim"` | Container image used for model download jobs Can be customized for local deployments with network restrictions Default: python:3.11-slim (public Docker Hub) Internal deployment uses: orbit-harbor.act.buaa.edu.cn/docker.io/library/python:3.11-slim |
...
| backendConfig.postgres | object | `{"TimeZone":"Asia/Shanghai","dbname":"postgres","host":"orbit-postgresql.orbit-system.svc.cluster.local","password":"<MUSTEDIT>","port":5432,"sslmode":"disable","user":"postgres"}` | PostgreSQL database connection configuration (Required) All fields must be specified for database connectivity |
...
| backendConfig.postgres.host | string | `"orbit-postgresql.orbit-system.svc.cluster.local"` | PostgreSQL server hostname or IP address (Required) Must be reachable from the application |
...
| backendConfig.secrets | object | `{"imagePullSecretName":"","tlsForwardSecretName":"orbit-tls-forward-secret","tlsSecretName":"orbit-tls-secret"}` | Kubernetes secret names for various security components (Required) All secret names must correspond to existing Kubernetes secrets |
...
| backendConfig.secrets.tlsForwardSecretName | string | `"orbit-tls-forward-secret"` | Name of the Kubernetes secret for TLS forwarding configuration (Required) Secret must contain appropriate forwarding certificates |
| backendConfig.secrets.tlsSecretName | string | `"orbit-tls-secret"` | Name of the Kubernetes secret containing TLS certificates for HTTPS (Required) Secret must contain 'tls.crt' and 'tls.key' keys |
...
| backendConfig.storage | object | `{"prefix":{"account":"accounts","public":"public","user":"users"},"pvc":{"readOnlyMany":null,"readWriteMany":"orbit-rw-storage"}}` | Persistent volume claim and path prefix configurations (Required) All PVC names and prefix paths must be specified |
...
| backendConfig.storage.pvc.readWriteMany | string | `"orbit-rw-storage"` | Name of the ReadWriteMany Persistent Volume Claim for shared storage (Required) PVC must exist in the cluster with ReadWriteMany access mode |
...
| firstUser | object | `{"password":"Masked@Password","username":"orbit-admin"}` | First user configuration When connecting to the database for the first time, creates the first user account with administrator privileges |
...
| firstUser.username | string | `"orbit-admin"` | Username for the first administrator user |
| frontendConfig | object | `{"grafana":{"job":{"basic":"/d/R4ZPFfyIz/orbit-job-basic-dashboard","nvidia":"/d/2CDE0AC7/orbit-job-nvidia-dashboard","pod":"/d/MhnFUFLSz/orbit-pod-dashboard"},"node":{"basic":"/d/k8s_views_nodes/orbit-node-basic-dashboard","nvidia":"/d/nvidia-dcgm-dashboard/orbit-node-nvidia-dashboard"},"overview":{"main":"/d/f33ade9f-821d-4e96-a7f2-eb16c8b9c447/838ffad","network":"/d/8b7a8b326d7a6f1f04y7fh66368c67af/networking","schedule":"/d/be9oh7yk24jy8f/orbit-gpu-e8b083-e5baa6-e58f82-e88083"},"user":{"nvidia":"/d/user-nvidia-dcgm-dashboard/orbit-user-nvidia-dashboard"}},"url":{"apiPrefix":"/api/v1","document":"https://raids-lab.github.io/orbit/zh"},"version":"1.0.0"}` | Frontend configuration |
| frontendConfig.grafana | object | `{"job":{"basic":"/d/R4ZPFfyIz/orbit-job-basic-dashboard","nvidia":"/d/2CDE0AC7/orbit-job-nvidia-dashboard","pod":"/d/MhnFUFLSz/orbit-pod-dashboard"},"node":{"basic":"/d/k8s_views_nodes/orbit-node-basic-dashboard","nvidia":"/d/nvidia-dcgm-dashboard/orbit-node-nvidia-dashboard"},"overview":{"main":"/d/f33ade9f-821d-4e96-a7f2-eb16c8b9c447/838ffad","network":"/d/8b7a8b326d7a6f1f04y7fh66368c67af/networking","schedule":"/d/be9oh7yk24jy8f/orbit-gpu-e8b083-e5baa6-e58f82-e88083"},"user":{"nvidia":"/d/user-nvidia-dcgm-dashboard/orbit-user-nvidia-dashboard"}}` | Grafana dashboard configurations References: https://github.com/raids-lab/orbit/tree/main/grafana-dashboards |
| frontendConfig.grafana.job.basic | string | `"/d/R4ZPFfyIz/orbit-job-basic-dashboard"` | Basic job dashboard URL |
| frontendConfig.grafana.job.nvidia | string | `"/d/2CDE0AC7/orbit-job-nvidia-dashboard"` | NVIDIA job dashboard URL |
| frontendConfig.grafana.job.pod | string | `"/d/MhnFUFLSz/orbit-pod-dashboard"` | Pod dashboard URL |
| frontendConfig.grafana.node.basic | string | `"/d/k8s_views_nodes/orbit-node-basic-dashboard"` | Basic node dashboard URL |
| frontendConfig.grafana.node.nvidia | string | `"/d/nvidia-dcgm-dashboard/orbit-node-nvidia-dashboard"` | NVIDIA node dashboard URL |
...
| frontendConfig.grafana.overview.schedule | string | `"/d/be9oh7yk24jy8f/orbit-gpu-e8b083-e5baa6-e58f82-e88083"` | Schedule dashboard URL |
| frontendConfig.grafana.user.nvidia | string | `"/d/user-nvidia-dcgm-dashboard/orbit-user-nvidia-dashboard"` | User NVIDIA dashboard URL |
| frontendConfig.url.apiPrefix | string | `"/api/v1"` | Backend API prefix (not modifiable currently) |
| frontendConfig.url.document | string | `"https://raids-lab.github.io/orbit/zh"` | Documentation base URL |
...
| host | string | `"orbit.example.com"` | Domain name or IP address that the server will bind to (Required) Must be specified for the server to start |
| imagePullPolicy | string | `"Always"` | Image pull policy ("IfNotPresent" | "Always", for development, use Always) |
| imagePullSecrets | list | `[]` | Image pull secrets |
| images | object | `{"backend":{"repository":"ghcr.io/raids-lab/orbit-backend","tag":"latest"},"buildkit":{"repository":"docker.io/moby/buildkit","tag":"v0.23.1"},"buildx":{"repository":"ghcr.io/raids-lab/buildx-client","tag":"latest"},"cronjob":{"repository":"docker.io/badouralix/curl-jq","tag":"latest"},"dbBackup":{"repository":"docker.io/library/postgres","tag":"16.4"},"envd":{"repository":"ghcr.io/raids-lab/envd-client","tag":"latest"},"frontend":{"repository":"ghcr.io/raids-lab/orbit-frontend","tag":"latest"},"grafanaProxy":{"repository":"docker.io/library/nginx","tag":"1.27.3-bookworm"},"nerdctl":{"repository":"ghcr.io/raids-lab/nerdctl-client","tag":"latest"},"storage":{"repository":"ghcr.io/raids-lab/storage-server","tag":"latest"}}` | Container images configuration |
| images.backend.repository | string | `"ghcr.io/raids-lab/orbit-backend"` | Backend service image repository |
...
| images.frontend.repository | string | `"ghcr.io/raids-lab/orbit-frontend"` | Frontend service image repository |
...
| namespaces | object | `{"create":true,"image":"orbit-images","job":"orbit-workspace"}` | Namespace configuration for orbit components By default, orbit components run in orbit namespace, while jobs and images are in separate namespaces |
| namespaces.create | bool | `true` | Whether to create namespaces along with the deployment |
| namespaces.image | string | `"orbit-images"` | Namespace for building images |
| namespaces.job | string | `"orbit-workspace"` | Namespace for running jobs |
...
| storage | object | `{"create":true,"pvcName":"orbit-rw-storage","request":"10Gi","storageClass":"nfs"}` | Persistent Volume Claim configuration |
| storage.create | bool | `true` | Whether to create PVC or use existing pvc. |
| storage.pvcName | string | `"orbit-rw-storage"` | PVC name (existing or created, also used in backendConfig) |
...
| tls.base | object | `{"cert":"<MASKED>","create":false,"key":"<MASKED>"}` | Base certificate configuration (Standard mode, e.g., orbit.example.com certificate) |
...
| tls.forward | object | `{"cert":"<MASKED>","create":false,"key":"<MASKED>"}` | Forward certificate configuration (Subdomain mode, e.g., xxx.orbit.example.com certificate for exposing internal job services externally) |
