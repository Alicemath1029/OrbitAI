...
export const CHART_YAML_URL =
  'https://github.com/raids-lab/orbit/blob/main/charts/orbit/Chart.yaml';

const PLACEHOLDER = '<chart-version>';

/**
 * Returns the Orbit Helm Chart version from build-time env.
 * Empty or placeholder value means "not injected" (e.g. build without Chart.yaml).
 */
export function getChartVersion(): string {
  const v = process.env.NEXT_PUBLIC_ORBIT_CHART_VERSION;
