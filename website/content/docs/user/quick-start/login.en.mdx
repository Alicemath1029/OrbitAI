...
description: Log in to Orbit using an existing account or create a Orbit account
---

To use the Orbit platform, you need to have a valid platform account. Depending on the platform configuration, you can choose to log in using the laboratory's LDAP account or through a Orbit platform account.
...
### Overview
Orbit supports automatic registration and information synchronization of LDAP accounts. You don't need to manually create an account; just enter your LDAP username and password to log in.
...
flowchart TD;
    ...
    B -- Auth Success --> D{Does user exist<br/>in Orbit DB?};
    D -- No --> E1[Get User UID/GID<br/>from LDAP attributes or external service];
    E1 --> E2[Automatically create account<br/>and sync attributes in Orbit DB];
    D -- Yes --> F[Update Orbit DB attributes<br/>such as Email, Nickname, etc.];
...
### Overview
If you have manually registered an account in Orbit, or if the administrator assigned you an account and requested this login method, please log in this way.
...
<Mermaid
  chart="
flowchart TD;
    A[Enter Orbit Account Username/Password] --> B{Orbit DB Comparison<br/>Bcrypt Validation};
...
### Overview
When the administrator has enabled the "Allow Self-Registration" function, new users who do not yet have any account can create a Orbit platform account through the registration page.
...
flowchart TD;
    ...
    D -- Does Not Exist --> F1[Assign Orbit Default UID/GID 1001];
    F1 --> F2[Create User Record in Orbit DB];
