...
**转发路径**：所有的访问路径均采用统一格式：`orbit.act.buaa.edu.cn/ingress/{userName}-{uuid}`。其中，`userName` 是用户名，`uuid` 为自动生成的五位标识符，指向特定的服务。

![ingress-intro](./img/ingress-intro.webp)

设置完成后会在对应 Pod 的 `Annotations` 中看到如下内容，使用 `ingress.orbit.raids.io` 作为 `key` 值：
...
- **访问路径** (`prefix`): 访问路径会被映射为 `orbit.act.buaa.edu.cn/ingress/{userName}-{uuid}`，其其中，`userName` 是用户名，`uuid`为 自动生成的五位标识符。
