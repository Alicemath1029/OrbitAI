---
title: "設定の説明"
description: "Orbit プラットフォーム Helm Chart の詳細な設定パラメータの説明。バックエンド、フロントエンド、ストレージ、監視、認証などのコアモジュールを網羅しています。"
---

<ChartBadge />

Orbit は Kubernetes 向けに設計された総合的な AI 開発プラットフォームであり、GPU リソース管理、コンテナ化された開発環境、およびワークフローのオーケストレーション機能を提供します。本ドキュメントでは、Helm を使用して Orbit をデプロイする際の設定可能なすべての項目について詳しく説明します。

**プロジェクトホームページ：** [https://github.com/raids-lab/orbit](https://github.com/raids-lab/orbit)
...
| パラメータ | 型 | デフォルト値 | 説明 |
|-----|------|---------|-------------|
| host | string | `"orbit.example.com"` | プラットフォームにアクセスするためのドメイン名または IP アドレス |
| protocol | string | `"http"` | アクセスプロトコルの種類（`http` または `https`） |
| firstUser.username | string | `"orbit-admin"` | 初回管理者ユーザー名 |
...
| namespaces.job | string | `"orbit-workspace"` | ジョブタスクを実行するための名前空間 |
| namespaces.image | string | `"orbit-images"` | イメージをビルドするための名前空間 |
...
| storage.pvcName | string | `"orbit-rw-storage"` | 共有 PVC の名前 |
...
| パラメータ | 型 | デフォルト値 | 説明 |
|-----|------|---------|-------------|
| backendConfig.secrets.tlsSecretName | string | `"orbit-tls-secret"` | HTTPS 用の TLS 証明書 Secret 名 |
| backendConfig.secrets.tlsForwardSecretName | string | `"orbit-tls-forward-secret"` | 転送用の TLS 証明書 Secret 名 |
