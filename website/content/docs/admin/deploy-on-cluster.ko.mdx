...
Ingress 와 도메인 해석이 설정된 경우, 실제 도메인 (예: `orbit.mycluster.local`)을 사용할 수 있습니다.  
...
storage:
  ...
  pvcName: "orbit-rw-storage" # 백엔드 마운트용 공유 PVC 이름
```
...
backendConfig:
  postgres:
    host: orbit-postgresql.cnpg-system.svc.cluster.local  # CloudNativePG 클러스터의 서비스 이름
    port: 5432
    dbname: postgres
    user: postgres
    password: <MUSTEDIT>
...
>     
> - CloudNativePG 의 기본 클러스터 이름이 `orbit-postgresql`인 경우, 서비스 이름은 다음과 같아야 합니다:  
>     `orbit-postgresql-rw.cnpg-system.svc.cluster.local`입니다.
>     
...
조직에서 이미 LDAP 디렉토리 서비스(예: OpenLDAP 또는 Active Directory)를 사용 중인 경우, Orbit를 해당 서비스에 연결하여 통합 인증을 구현할 수 있습니다.
