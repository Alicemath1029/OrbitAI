---
title: Volcano 集成指南
description: Orbit 如何使用 Volcano 批处理调度器进行多实验室 GPU 资源管理及分布式工作负载。

---

## 概述

[Volcano](https://volcano.sh/zh/) 是一个为高性能工作负载（如 AI/ML 训练、大数据和科学计算）设计的批处理调度系统。在 Orbit 中，Volcano 以公平且抢占的方式管理多个实验室和用户之间的 GPU 调度。

---

## 为什么选择 Volcano？

我们选择 Volcano 用于 Orbit，是因为它丰富的调度功能、可扩展的插件系统，以及对 **分布式训练**、**公平资源共享** 和 **任务级控制** 的原生支持。

Orbit 使用了以下 Volcano 组件：
...
Orbit 面向的是一个学术多租户场景，其中 GPU 集群由多个研究实验室共享。为了管理资源公平性：
...
Orbit 使用 Volcano 的 [Job CRD](https://volcano.sh/zh/docs/job-tutorial/) 支持：
...
我们建议通过 Helm 并使用 Orbit 的预配置值来安装 Volcano。
