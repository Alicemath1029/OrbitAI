---
title: Harbor 배포 가이드
description: Orbit 는 클러스터 내에서 이미지 빌드, 저장 및 보안 분배를 지원하기 위해 내부 Harbor 레지스트리를 사용합니다. 
...
도메인 이름 (예: orbit-harbor.act.buaa.edu.cn) 이 클러스터 내부 또는 내부 DNS 에서 해석될 수 있어야 합니다.
...
## 이미지 풀 시크릿
Orbit 구성 요소가 Harbor 에서 이미지를 끌어오도록 허용하려면:
```bash
...
| 프로젝트       | 목적                    |
| ------------- | -------------------------- |
| orbit/base   | 기본 OS 및 유틸리티      |
| orbit/jobs   | 사용자가 제출한 작업 이미지 |
| orbit/charts | Helm 차트의 미러         |
| orbit/models | 추론을 위한 대규모 모델  |
