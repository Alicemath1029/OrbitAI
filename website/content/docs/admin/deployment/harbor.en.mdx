...
description: Orbit relies on an internal Harbor registry to support image building, storage, and secure distribution within the cluster. 
...
The domain name (e.g., orbit-harbor.act.buaa.edu.cn) is resolvable within your cluster or internal DNS.
...
## Image Pull Secrets
To allow Orbit components to pull images from Harbor:
```bash
...
| Project       | Purpose                    |
| ------------- | -------------------------- |
| orbit/base   | Base OS and utilities      |
| orbit/jobs   | User-submitted job images  |
| orbit/charts | Mirrors for Helm charts    |
| orbit/models | Large models for inference |
