...
description: Orbit は、クラスター内でイメージのビルド、ストレージ、セキュアな配布をサポートするために、内部の Harbor レジストリに依存しています。
...
ドメイン名（例：orbit-harbor.act.buaa.edu.cn）はクラスターまたは内部 DNS 内で解決可能である必要があります。
...
## イメージプルシークレット
Orbit コンポーネントが Harbor からイメージをプルできるようにするには：
```bash
...
| プロジェクト       | 目的                    |
| ------------- | -------------------------- |
| orbit/base   | ベース OS とユーティリティ      |
| orbit/jobs   | ユーザーが送信したジョブイメージ |
| orbit/charts | Helm チャートのミラー        |
| orbit/models | 推論用の大規模モデル        |
