...
Orbit 는 **kube-prometheus-stack**에 의존하여 견고하고 확장 가능한 모니터링 솔루션을 제공합니다. 다음과 같은 실시간 및 역사적 가시성을 제공합니다:
...
Orbit 는 Grafana 대시보드를 **자신의 프론트엔드에 직접 통합**하여 플랫폼 UI 를 벗어나지 않고도 사용자에게 다차원의 통찰을 제공합니다.
...
Orbit 모니터링을 완전히 활성화하려면 다음 구성 요소가 **먼저 설치되어 있어야 합니다**:
...
- DCGM 메트릭 수집 활성화
- Orbit 작업 부하에 관련된 Grafana 대시보드 사전 로드
- Grafana 서비스 유형을 `ClusterIP`로 설정 및 Orbit 관리 ingress 와 연동
...
Orbit 의 사전 구성된 값과 함께 공식 Helm 차트를 사용하는 것을 권장합니다.
