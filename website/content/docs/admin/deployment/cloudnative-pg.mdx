...
description: "使用 CloudNativePG Operator 和 OpenEBS 存储将 PostgreSQL 集成到 Orbit 中。"
---

## 概述

Orbit 使用 **PostgreSQL** 作为其后端数据库，用于管理用户会话、任务元数据、调度状态和平台配置。
...
在 Orbit 栈中，每个 PostgreSQL 集群都作为自定义的 `Cluster` 资源进行定义，并由操作符控制。
