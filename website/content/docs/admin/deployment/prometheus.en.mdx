...
Orbit relies on the **kube-prometheus-stack** to provide a robust and extensible monitoring solution. It powers real-time and historical observability for:
...
Orbit integrates Grafana dashboards **directly into its frontend**, providing users with multi-dimensional insights without leaving the platform UI.
...
To fully enable Orbit monitoring, please ensure the following components are **installed first**:
...
- Preloading a set of Grafana dashboards relevant to Orbit workloads
- Setting Grafana service type to `ClusterIP` with Orbit-managed ingress
...
We recommend using the official Helm chart with Orbit's preconfigured values.
