...
description: OpenEBS is used in Orbit for managing local persistent storage via Local PV HostPath volumes, enabling efficient node-local storage management.
---

## Overview

Orbit relies on **OpenEBS** to manage **local persistent storage** across Kubernetes nodes. Our main goal is to:

- **Enable CRD-based management of local storage resources**, allowing Orbit to treat local disks as first-class, declarative resources.
...
We provide a customized Helm configuration for OpenEBS to suit Orbit's storage needs.
