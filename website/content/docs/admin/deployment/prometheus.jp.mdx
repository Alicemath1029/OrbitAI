...
Orbit は **kube-prometheus-stack** を使って、拡張性のある堅牢な監視ソリューションを提供します。以下についてリアルタイムおよび歴史的な可観測性を提供します：
...
Orbit は **フロントエンドに Grafana ダッシュボードを直接統合**しており、プラットフォーム UI を離れる必要なく、多様な洞察を提供します。
...
Orbit の監視を完全に有効にするには、以下のコンポーネントを**まずインストールしてください**:
...
- Orbit ワークロードに関連する Grafana ダッシュボードの事前読み込み
- Grafana サービスタイプを `ClusterIP` に設定し、Orbit 管理の Ingress を使用
...
Orbit の事前に設定された値を使用した公式の Helm チャートの使用を推奨します。
