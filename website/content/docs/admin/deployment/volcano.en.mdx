...
description: How Orbit uses Volcano batch scheduler for multi-lab GPU resource management and distributed workloads.
---

## Overview

[Volcano](https://volcano.sh/en/) is a batch scheduling system designed for high-performance workloads such as AI/ML training, big data, and scientific computing. In Orbit, Volcano is used to manage GPU scheduling across multiple labs and users in a fair and preemptive way.

---

## Why Volcano?

We chose Volcano for Orbit due to its rich scheduling capabilities, extensible plugin system, and native support for **distributed training**, **fair resource sharing**, and **job-level control**.

Orbit leverages the following Volcano components:
...
Orbit serves on an academic multi-tenant senario, where GPU clusters are shared by various research labs. To manage resource fairness:
...
Orbit uses Volcano’s [Job CRD](https://volcano.sh/en/docs/job-tutorial/) to support:
...
We recommend installing Volcano via Helm with Orbit’s preconfigured values.
