...
description: "Integrate PostgreSQL into Orbit using the CloudNativePG Operator with OpenEBS storage."
---

## Overview

Orbit uses **PostgreSQL** as its backend database to manage user sessions, job metadata, scheduling state, and platform configurations.
...
In the Orbit stack, each PostgreSQL cluster is defined as a custom `Cluster` resource and controlled by the operator.
