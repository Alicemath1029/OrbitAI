---
title: 저장소 아키텍처
description: Orbit 는 고성능 로컬 워크로드 및 Pod 와 노드 간 지속 가능한 공유 데이터 액세스를 위해 하이브리드 저장소 아키텍처를 사용합니다. 이 문서는 클러스터에서 사용되는 저장소 솔루션에 대해 설명합니다.
---
...
지속 가능한, 다노드 접근 가능한 볼륨을 위해 Orbit 는 [Rook-Ceph RBD](https://rook.io/docs/rook/latest/ceph-block.html)를 사용합니다. RBD 볼륨은 동적으로 프로비저닝되며 다음을 지원합니다:

- 마이그레이션 지원을 통한 ReadWriteOnce 액세스
- 복제 및 장애 복구
- 다음에 적합합니다:
  - Prometheus TSDB 저장소
  - 지속성을 요구하는 Orbit 내부 서비스
  - 노드 간 공유되는 데이터세트
...
| 구성 요소             | 유형               | 저장소 백엔드       | 설명                                   |
|-----------------------|--------------------|----------------------|----------------------------------------|
| PostgreSQL (Orbit DB) | 지속 가능          | LocalPV (OpenEBS)    | 고속, 노드 특정                        |
...
NFS 를 저장소 프로비저너로 사용하고 Orbit 의 사전 구성된 값과 함께 공식 Helm 차트를 사용하는 것을 권장합니다.
