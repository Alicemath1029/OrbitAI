...
description: Orbit utilizes a hybrid storage architecture to address both high-performance local workloads and persistent shared data access across pods and nodes. This document outlines the storage solutions used in the cluster.
---
...
For persistent, multi-node accessible volumes, Orbit uses [Rook-Ceph RBD](https://rook.io/docs/rook/latest/ceph-block.html). RBD volumes are dynamically provisioned and support:

- ReadWriteOnce access with migration support
- Replication and failure recovery
- Suitable for:
  - Prometheus TSDB storage
  - Orbit internal services requiring persistence
...
|-----------------------|--------------------|----------------------|----------------------------------------|
| PostgreSQL (Orbit DB) | Persistent         | LocalPV (OpenEBS)    | High-speed, node-specific              |
...
We recommend leveraging NFS as the storage provisioner and using official Helm chart with Orbit's preconfigured values.
