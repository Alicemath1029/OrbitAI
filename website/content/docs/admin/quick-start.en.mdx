...
- **NVIDIA GPU Operator**: Responsible for installing GPU drivers, device plugins, and monitoring components, ensuring that Orbit can schedule GPU tasks.
- **Bitnami PostgreSQL**: A PostgreSQL database service without high availability, mainly used as an external database for Orbit in this scheme.
...
- **Volcano**: A scheduling framework for batch processing and AI workloads, which is the core scheduling component of Orbit.
...
helm upgrade orbit oci://ghcr.io/raids-lab/orbit --version <chart-version>
```
...
git clone https://github.com/raids-lab/orbit.git
cd orbit/charts

# Install the chart
helm install orbit orbit/
```
...
```bash
helm install orbit oci://ghcr.io/raids-lab/orbit --values values.yaml
```
...
Orbit relies on Volcano to provide batch computing scheduling capabilities for task execution and scheduling. Volcano is optimized for AI and big data scenarios, supporting features such as queues, priorities, and gang scheduling. Even in the minimal deployment environment, Volcano is an essential core component; otherwise, Orbit cannot schedule and run tasks.
...
value.yaml configuration reference: https://github.com/raids-lab/orbit-backend/blob/main/deployments/volcano/values.yaml
...
Harbor is the image repository that the Orbit platform depends on. In an intranet or offline environment, public image repositories (such as DockerHub, ghcr.io) are often inaccessible, so a private repository must be used to store the frontend and backend images of Orbit and its dependencies.
 ...
 This not only ensures that Orbit's images can be stored and distributed locally, but also provides a foundation for future expansion (such as multi-user and multi-project image management).
