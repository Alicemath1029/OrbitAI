...
metadata:
  name: hostdevice-net
spec:
  networkNamespace: "orbit-workspace" # 워크로드가 있는 네임스페이스
  resourceName: "hostdev"
  ipam: |
    {
      "type": "whereabouts",
...
metadata:
  name: testpod1
  namespace: orbit-workspace. # 이전에 지정한 네임스페이스
  annotations:
...
RDMA 대역폭 테스트 시 작은 데이터(예: 1024 바이트) 전송은 정상이지만, 1M 이상의 데이터 전송은 실패합니다. 구체적인 오류 메시지는 위와 같습니다. 자세한 내용은 [Issue #339](https://github.com/raids-lab/orbit/issues/339)를 참조하십시오.
...
Orbit 는 이미 작업에 `CAP_IPC_LOCK`을 설정하여 메모리 잠금 작업을 수행할 수 있도록 허용했지만, 잠글 수 있는 양에 대한 제한은 수정하지 않았습니다. 또한 컨테이너에 `CAP_SYS_RESOURCE` 특권이 없으므로 컨테이너 내부 작업으로는 이 상한을 변경할 수 없습니다.
...
2.  **드라이버 환경 검증 및 보고 (MOFED 드라이버 Pod)**：Operator 는 각 노드에서 `mofed-driver` Pod 를 스케줄하고 실행하려고 시도합니다. 이 Pod 는 호스트 Mellanox 드라이버의 검사, 설치, 업데이트를 담당합니다. 이 Pod 가 해당 노드에서 성공적으로 실행되어 준비 완료 신호를 반환한 후에만 Operator 가 `mofed.wait` 레이블을 `false` 로 업데이트합니다.
    *   **핵심 차단 지점**：노드에 **사용자 정의 비즈니스 테인트**(예: `orbit.raids.io/account` 전용)가 설정되었거나 **유지보수 모드**(Cordon 됨, `node.kubernetes.io/unschedulable` 테인트 보유)인데, `NicClusterPolicy` 에 해당 **Tolerations(허용)** 이 구성되어 있지 않으면 드라이버 Pod 가 스케줄러에 의해 차단됩니다. 이로 인해 흐름이 이 단계에서 멈추고 노드 레이블이 영구적으로 `true` 로 유지되어 리소스 발견이 잠깁니다.
