...
metadata:
  name: hostdevice-net
spec:
  networkNamespace: "orbit-workspace" # Workloads 所在的命名空间
  resourceName: "hostdev"
  ipam: |
    {
      "type": "whereabouts",
...
metadata:
  name: testpod1
  namespace: orbit-workspace. # 前面指定的命名空间
  annotations:
...
RDMA 进行带宽测试时，小数据量（如 1024 字节）传输正常，但 1M 及以上数据量传输失败，具体的报错信息与上面类似，详见 [Issue #339](https://github.com/raids-lab/orbit/issues/339)。
...
Orbit 已经通过给作业设置 `CAP_IPC_LOCK` 允许其执行锁定内存的操作，但是没有修改允许锁定的量的限制。同时，由于没有给予容器 `CAP_SYS_RESOURCE` 的权限，容器内部的操作也无法修改这个上限。
...
## RDMA 相关工具 Pod 的对 Orbit 污点的容忍问题
...
2.  **驱动环境校验与上报 (MOFED 驱动 Pod)**：Operator 尝试在每个节点上调度并运行 `mofed-driver` Pod。该 Pod 负责检查、安装或更新宿主机的 Mellanox 驱动。只有当该 Pod 在对应节点成功运行并返回就绪信号后，Operator 才会将 `mofed.wait` 标签更新为 `false`。
    *   **核心卡点**：如果节点设置了**自定义业务污点**（如 `orbit.raids.io/account` 独占）或处于**维护模式**（被 Cordon，带有 `node.kubernetes.io/unschedulable` 污点），且 `NicClusterPolicy` 中未配置相应的**容忍度 (Tolerations)**，则驱动 Pod 会被调度器拦截。这会导致流程卡在这一步，节点标签永久停留在 `true`，从而锁死资源发现。
