...
metadata:
  name: hostdevice-net
spec:
  networkNamespace: "orbit-workspace" # Workloads が存在する名前空間
  resourceName: "hostdev"
  ipam: |
    {
      "type": "whereabouts",
...
metadata:
  name: testpod1
  namespace: orbit-workspace. # 以前に指定した名前空間
  annotations:
...
RDMA の帯域幅テストを行う際、小規模なデータ（1024 バイトなど）の転送は正常ですが、1M 以上のデータの転送に失敗します。具体的なエラーメッセージは上記と同様です。詳細は [Issue #339](https://github.com/raids-lab/orbit/issues/339) を参照してください。
...
Orbit は、ジョブに `CAP_IPC_LOCK` を設定することでメモリロック操作の実行をすでに許可していますが、許可されるロック量の制限は変更していません。また、コンテナーに `CAP_SYS_RESOURCE` 特権が付与されていないため、コンテナー内部の操作ではこの上限を変更できません。
...
## RDMA 関連ツール Pod の Orbit テイントへの許容
...
2.  **ドライバ環境検証と報告 (MOFED ドライバ Pod)**：Operator は各ノードで `mofed-driver` Pod のスケジュールと実行を試みます。この Pod はホストの Mellanox ドライバのチェック、インストール、更新を担当します。この Pod が対応ノードで正常に実行され、準備完了シグナルを返した後にのみ、Operator は `mofed.wait` ラベルを `false` に更新します。
    *   **コアブロックポイント**：ノードに**カスタム業務テイント**（例：`orbit.raids.io/account` 専用）が設定されているか、**メンテナンスモード**（Cordon 済み、`node.kubernetes.io/unschedulable` テイント付き）の場合、かつ `NicClusterPolicy` に該当する**許容 (Tolerations)** が設定されていないと、ドライバ Pod はスケジューラによってブロックされます。これによりフローがこのステップで停止し、ノードラベルが永続的に `true` のままとなり、リソース検出がロックされます。
