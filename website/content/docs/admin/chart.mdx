---
title: "配置说明"
description: "Orbit 平台 Helm Chart 详细配置参数说明，涵盖后端、前端、存储、监控及认证等核心模块。"
---

<ChartBadge />

Orbit 是一个专为 Kubernetes 设计的综合性 AI 开发平台，提供 GPU 资源管理、容器化开发环境和工作流编排功能。本文档详细列出了通过 Helm 部署 Orbit 时的所有可配置项。

**项目主页：** [https://github.com/raids-lab/orbit](https://github.com/raids-lab/orbit)
...
| 参数 | 类型 | 默认值 | 描述 |
|-----|------|---------|-------------|
| host | string | `"orbit.example.com"` | 平台访问的域名或 IP 地址 |
| protocol | string | `"http"` | 访问协议类型（`http` 或 `https`） |
| firstUser.username | string | `"orbit-admin"` | 初始管理员用户名 |
...
| namespaces.job | string | `"orbit-workspace"` | 用于运行作业任务的命名空间 |
| namespaces.image | string | `"orbit-images"` | 用于构建镜像的命名空间 |
...
| storage.pvcName | string | `"orbit-rw-storage"` | 共享 PVC 的名称 |
...
| 参数 | 类型 | 默认值 | 描述 |
|-----|------|---------|-------------|
| backendConfig.secrets.tlsSecretName | string | `"orbit-tls-secret"` | 用于 HTTPS 的 TLS 证书 Secret 名称 |
| backendConfig.secrets.tlsForwardSecretName | string | `"orbit-tls-forward-secret"` | 用于转发的 TLS 证书 Secret 名称 |
