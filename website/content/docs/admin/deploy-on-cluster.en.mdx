---
title: Orbit Cluster Deployment Guide
description: This guide outlines the standardized deployment process of the Orbit platform on a production-grade Kubernetes cluster, including environment preparation, dependency component installation, core service deployment, and common troubleshooting.
---
...
Orbit platform relies on the following foundational components to enable scheduling, monitoring, networking, and storage functionality.  
...
**Function:** Provides external access points for Orbit frontend and API.
...
```bash
helm pull oci://ghcr.io/raids-lab/orbit --version <chart-version> --untar
```

The core configuration file for the Orbit platform is `values.yaml`.  
...
If Ingress and DNS are configured, use a real domain name (e.g., `orbit.mycluster.local`).  
...
storage:
  ...
  pvcName: "orbit-rw-storage" # Name of the shared PVC mounted behind
```

The Orbit backend will automatically mount this PVC for user space and public directories (`users/`, `accounts/`, `public/`).

---

### 4.3 PostgreSQL Database Configuration (CloudNativePG)

Orbit uses a database cluster deployed via CloudNativePG.  
Set the database connection parameters to point to the corresponding service:

```yaml
backendConfig:
  postgres:
    host: orbit-postgresql.cnpg-system.svc.cluster.local  # Service name of CloudNativePG Cluster
    port: 5432
    dbname: postgres
    user: postgres
    password: <MUSTEDIT>
...
> 
> - If the default CloudNativePG cluster name is `orbit-postgresql`, the service name should be:  
>     `orbit-postgresql-rw.cnpg-system.svc.cluster.local`.

---

### 4.4 Monitoring System Configuration (Prometheus Stack)

Orbit backend retrieves GPU and job metrics via the Prometheus API.  
...
After enabling, Orbit can automatically push built images to the Harbor registry.
...
Orbit exposes services via Ingress by default.  
...
```bash
kubectl create secret tls orbit-tls-secret \
  --cert=tls.crt --key=tls.key -n orbit-system
```
...
If your organization already has an existing LDAP directory service (such as OpenLDAP or Active Directory), you can configure Orbit to integrate with this service for unified identity authentication.
...
```bash
helm install orbit oci://ghcr.io/raids-lab/orbit \
...
```bash
kubectl get pods -n orbit-system
kubectl get ingress -n orbit-system
```
...
This document applies to Orbit v0.1.0 and above.
