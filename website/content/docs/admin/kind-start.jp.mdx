---
title: 最小化デプロイ
description: このドキュメントでは、Kind を使用してローカルで Orbit 環境を迅速に構築する方法をガイドします。Orbit は Kubernetes ベースの分散トレーニングプラットフォームであり、本ガイドでは Kind クラスターの作成から Orbit の必要な各コンポーネントのデプロイに至るまでの完全な手順をカバーしています。
---
...
<Callout type="info">
    データベースパスワードを適切に保存してください。後続の Orbit のデプロイ時に必要になります。
</Callout>
...
### 7.1 Orbit Helm Chart の値ファイルの取得
...
<Callout type="success">
    すべての Pod が Running 状態であれば、Orbit 環境の構築は成功しています！
</Callout>
...
本ガイドに従って、Kind クラスター上に完全な Orbit 環境をデプロイしました。含まれる主なコンポーネントは以下の通りです：
...
- ✅ Volcano スケジューラ
- ✅ NFS ストレージプロビジョナー
- ✅ Orbit トレーニングプラットフォーム

今や、Orbit を使用して分散トレーニングタスクを開始できます！何か問題が発生した場合は、各コンポーネントの公式ドキュメントを参照し、ログを確認してトラブルシューティングを行ってください。