---
title: "설정 안내"
description: "Orbit 플랫폼 Helm Chart 상세 설정 파라미터 설명. 백엔드, 프론트엔드, 스토리지, 모니터링 및 인증 등 핵심 모듈을 다룹니다."
---

<ChartBadge />

Orbit 는 Kubernetes 를 위해 설계된 종합 AI 개발 플랫폼으로, GPU 자원 관리, 컨테이너화된 개발 환경 및 워크플로우 오케스트레이션 기능을 제공합니다. 이 문서는 Helm 을 통해 Orbit 를 배포할 때 설정 가능한 모든 항목을 상세히 설명합니다.

**프로젝트 홈페이지:** [https://github.com/raids-lab/orbit](https://github.com/raids-lab/orbit)
...
| 파라미터 | 타입 | 기본값 | 설명 |
|-----|------|---------|-------------|
| host | string | `"orbit.example.com"` | 플랫폼 접속을 위한 도메인 이름 또는 IP 주소 |
| protocol | string | `"http"` | 접속 프로토콜 유형 (`http` 또는 `https`) |
| firstUser.username | string | `"orbit-admin"` | 초기 관리자 사용자 이름 |
...
| namespaces.job | string | `"orbit-workspace"` | 작업 태스크 실행을 위한 네임스페이스 |
| namespaces.image | string | `"orbit-images"` | 이미지 빌드를 위한 네임스페이스 |
...
| storage.pvcName | string | `"orbit-rw-storage"` | 공유 PVC 이름 |
...
| 파라미터 | 타입 | 기본값 | 설명 |
|-----|------|---------|-------------|
| backendConfig.secrets.tlsSecretName | string | `"orbit-tls-secret"`| HTTPS 용 TLS 인증서 Secret 이름 ||
| backendConfig.secrets.tlsForwardSecretName | string | `"orbit-tls-forward-secret"` | 전달용 TLS 인증서 Secret 이름 |
